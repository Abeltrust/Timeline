    ...
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-md overflow-hidden border border-stone-100">
                <div class="relative">
                    <img src="{{ $stream->thumbnail ?? 'https://via.placeholder.com/400x200' }}" alt="Stream Thumbnail" class="w-full h-48 object-cover">
                    ...
                    <span class="absolute top-2 right-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs">{{ $stream->viewers_count }} viewers</span>
                </div>
                <div class="p-4 space-y-1">
                    <h3 class="font-semibold text-stone-800">{{ $stream->title }}</h3>
                    <p class="text-sm text-stone-500">Host: {{ $stream->host->name }}</p>
                    @if($stream->category)
                    <span class="inline-block bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded mt-1">{{ $stream->category }}</span>
                    @endif
                </div>
            </div>
            @endforeach
