        ...
        <div class="max-w-6xl mx-auto">
                ...
                @forelse ($communities as $community)
                    <div class="border border-stone-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition flex flex-col">
                        ...
                        <div class="h-32 sm:h-40 bg-gray-100">
                            @if($community->image)
                                <img src="{{ asset('storage/'.$community->image) }}" alt="{{ $community->name }}" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="p-4 sm:p-5 flex-1 flex flex-col justify-between">
                            <div class="space-y-1.5 sm:space-y-2">
                                <h2 class="text-base sm:text-lg font-semibold text-gray-800">{{ $community->name }}</h2>
                                <p class="text-xs sm:text-sm text-gray-600 line-clamp-2">{{ $community->description }}</p>
                            </div>
                            ...
                            <div class="mt-3 sm:mt-4 flex flex-wrap items-center justify-between gap-1.5 sm:gap-2 text-[11px] sm:text-sm text-gray-500">
                                ...
                                <span class="flex items-center gap-1">
                                    <i data-lucide="message-circle" class="w-3.5 h-3.5"></i> {{ $community->posts->count() }}
                                </span>
                                <span class="flex items-center gap-1">
                                    <i data-lucide="clock" class="w-3.5 h-3.5"></i> {{ $community->created_at->diffForHumans() }}
                                </span>
                            </div>
            ...
            <div class="mt-8 sm:mt-12">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    @forelse ($trending as $i => $story)
                        <div class="border border-stone-200 rounded-lg p-4 sm:p-5 hover:shadow-md transition">
                            <div class="flex items-center gap-2 sm:gap-3">
                                <span class="text-lg sm:text-2xl font-bold text-stone-400">#{{ $i+1 }}</span>
                                <div>
                                    <h3 class="text-sm sm:text-base font-semibold text-gray-800">{{ $story->title }}</h3>
                                    <p class="text-xs sm:text-sm text-gray-500">{{ $story->likes_count }} likes</p>
                                </div>
                            </div>
                        </div>
                    @empty
