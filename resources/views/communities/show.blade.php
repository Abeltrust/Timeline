...
<div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-md p-6 border border-stone-100">
            <form id="community-post-form" action="{{ route('communities.posts.store', $community->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <input type="hidden" name="community_id" value="{{ $community->id }}">
                <textarea name="content" rows="5" class="w-full p-4 border border-stone-300 rounded-2xl resize-none focus:ring-2 focus:ring-amber-300 text-sm sm:text-base placeholder-stone-400" placeholder="Share something with {{ $community->name }}..."></textarea>
        ...
        <div class="space-y-6" id="posts-feed">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-stone-100 p-5 post-card" id="post-{{ $post->id }}">
                ...
                <div class="flex items-center gap-3 mb-3">
                    <img src="{{ $post->user->profile_photo_url ?? asset('images/default-avatar.png') }}" alt="{{ $post->user->name }}" class="w-12 h-12 rounded-full">
                    <div>
                ...
                {{-- Post Content --}}
                <p class="mt-2 text-sm sm:text-base text-stone-700 leading-relaxed whitespace-pre-wrap">{{ $post->content }}</p>

                {{-- Media --}}
                <div class="mt-3 flex flex-col gap-4">
                    @if($post->image)
                    <img src="{{ asset('storage/'.$post->image) }}" class="rounded-xl w-full object-cover max-h-96 sm:max-h-[28rem]" alt="post image">
                    @endif
                    @if($post->video)
                ...
                <div class="flex items-center justify-center gap-6 mt-4 border-t border-stone-100 pt-3 flex-wrap">
                    @auth
                    <button title="Tap" data-tap-post="{{ $post->id }}" onclick="toggleCommunityTap({{ $post->id }}, this)" class="flex items-center gap-2 px-3 py-1 rounded-full text-sm transition {{ $post->taps()->where('user_id', auth()->id())->exists() ? 'bg-red-50 text-red-600' : 'text-stone-600 hover:bg-stone-50' }}">
                        <i data-lucide="heart" class="w-5 h-5 {{ $post->taps()->where('user_id', auth()->id())->exists() ? 'fill-current' : '' }}"></i>
                        <span id="tap-count-{{ $post->id }}" class="text-sm font-medium">{{ $post->taps()->count() }}</span>
                    </button>
                    @else
                    <a href="{{ route('login') }}" class="flex items-center gap-2 px-3 py-1 rounded-full text-sm text-stone-400 hover:text-amber-600 transition">
                        <i data-lucide="heart" class="w-5 h-5"></i>
                        <span class="text-sm font-medium">{{ $post->taps()->count() }}</span>
                    </a>
                    @endauth

                    <button title="Comments" onclick="toggleCommentsBox({{ $post->id }})" class="flex items-center gap-2 px-3 py-1 rounded-full text-sm text-stone-600 hover:bg-stone-50 transition">
                        <i data-lucide="message-square" class="w-5 h-5"></i>
                        <span id="comment-count-{{ $post->id }}" class="text-sm font-medium">{{ $post->comments()->count() }}</span>
                    </button>
                    ...
                    <button title="Share" onclick="openShareModal('{{ url('/communities/'.$community->id.'?post='.$post->id) }}', '{{ addslashes(Str::limit($post->content ?? '', 120)) }}')" class="flex items-center gap-2 px-3 py-1 rounded-full text-sm text-stone-600 hover:bg-stone-50 transition">
                ...
                <div id="comments-box-{{ $post->id }}" class="hidden mt-3 space-y-3">
                    ...
                    <div class="flex gap-3 items-start">
                        <img src="{{ $comment->user->profile_photo_url ?? asset('images/default-avatar.png') }}" class="w-9 h-9 rounded-full">
                        <div class="bg-stone-50 rounded-2xl p-4 flex-1">
                            ...
                            <div class="mt-2 text-xs text-stone-500 flex gap-3">
                                <button onclick="replyComment({{ $comment->id }})" class="hover:text-amber-600">Reply</button>
                            </div>
                        </div>
                    </div>
                    @endforeach
                    ...
                    <form onsubmit="submitComment(event, {{ $post->id }})" class="mt-3 flex items-center gap-3">
                        @csrf
                        <input id="comment-input-{{ $post->id }}" name="content" type="text" placeholder="Write a comment..." class="flex-1 px-4 py-3 rounded-full border border-stone-300 text-sm sm:text-base focus:ring-2 focus:ring-amber-300">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-md border border-stone-100">
            ...
            <div class="relative h-56 sm:h-64 lg:h-72 rounded-t-2xl overflow-hidden">
                <img src="{{ $community->image ? asset('storage/'.$community->image) : asset('images/community-default.jpg') }}" class="w-full h-full object-cover" alt="{{ $community->name }}">
            </div>

            {{-- Community Info --}}
            <div class="p-5">
                <h2 class="text-lg sm:text-xl font-semibold text-stone-800">{{ $community->name }}</h2>
                <p class="text-xs sm:text-sm text-stone-500 mt-1">{{ $community->members()->count() }} members • {{ $posts->total() ?? $community->communityPosts()->count() }} posts</p>
        ...
        <div class="bg-white rounded-2xl shadow-md border border-stone-100 p-5">
            ...
            <div class="grid grid-cols-4 gap-3">
                @foreach($community->members->take(8) as $member)
                    <img src="{{ $member->profile_photo_url ?? asset('images/default-avatar.png') }}" class="w-14 h-14 rounded-full mx-auto">
                @endforeach
            </div>
        </div>
