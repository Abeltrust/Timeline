...
<div class="max-w-6xl mx-auto p-4 sm:p-6">
    ...
    <div class="bg-gradient-to-br from-amber-50 via-orange-50 to-red-50 rounded-2xl p-6 sm:p-8 border border-amber-200 mb-8">
        ...
        <div class="text-center">
            ...
            <a href="{{ route('register') }}" class="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-6 sm:px-8 py-3 rounded-xl font-semibold hover:from-amber-600 hover:to-orange-700 transition-all duration-200 shadow-md hover:shadow-lg">
                Join Timeline
            </a>
        </div>
    </div>
            ...
            <article class="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-stone-100 hover:shadow-md transition-all duration-300">
                <div class="flex flex-col sm:flex-row items-start sm:space-x-4">
                    ...
                    <div class="flex-1 w-full">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3">
                            <div class="flex flex-wrap items-center gap-2 sm:gap-3 text-sm sm:text-base">
                                ...
                                <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center">
                                    <span class="text-white font-semibold text-sm">{{ $post->user->initials }}</span>
                                </div>

                                <!-- User Info -->
                                <a href="{{ route('profile.user', $post->user) }}" class="font-semibold text-stone-800 hover:text-amber-600">
                                    {{ $post->user->name }}
                                </a>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="mt-4 rounded-xl overflow-hidden" >
                                <img src="{{ Storage::url($post->image) }}" class="w-full max-h-[600px] object-contain  rounded-md w-full sm:max-h-[28rem]">
                            </div>
                            @endif
                        </div>
                   ...
                   <div class="flex flex-wrap justify-center items-center gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-stone-100 text-xs sm:text-sm md:text-base">
                        @auth
                            <!-- TAP -->
                            <button onclick="toggleTap({{ $post->id }})" 
                                data-tap-post="{{ $post->id }}"
                                ...
                                <i data-lucide="heart" class="w-3 h-3 sm:w-4 sm:h-4 {{ auth()->user()->hasTapped($post) ? 'fill-current' : '' }}"></i>
                            ...
                            <button onclick="toggleResonanceBox({{ $post->id }})"
                                ...
                                <span class="font-medium" data-resonance-count="{{ $post->id }}">{{ $post->resonances->count() }} </span>
                            </button>

                            <!-- Check-In -->
                            <button onclick="toggleCheckin({{ $post->id }})" 
                                data-checkin-post="{{ $post->id }}"
                                class="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg transition text-stone-600 hover:bg-stone-100">
                        ...
                        <div id="resonance-box-{{ $post->id }}" class="hidden mt-4">
                            <div class="space-y-2 sm:space-y-3">
                                ...
                                <div class="p-2 sm:p-3 bg-stone-50 rounded-lg">
                                    <span class="font-medium text-stone-800 text-xs sm:text-sm text-strong">{{ $resonance->user->name }}</span>
                                    <p class="text-stone-700 text-xs sm:text-sm leading-snug">{{ $resonance->content }}</p>
                                    <span class="text-[10px] sm:text-xs text-stone-500">{{ $resonance->created_at->diffForHumans() }}</span>
                                </div>
                                @endforeach
                            </div>
                           ...
                           <div class="flex items-center mt-2 sm:mt-3 space-x-1 sm:space-x-2">
                                <input type="text" id="resonance-input-{{ $post->id }}" 
