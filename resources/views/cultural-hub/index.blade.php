                ...
                <div class="lg:hidden flex items-center justify-between backdrop-blur-xl bg-white/80 dark:bg-stone-900/80 p-5 rounded-[2rem] border border-white/20 dark:border-stone-800/50 shadow-2xl relative z-40 mb-8">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-base font-black text-stone-900 dark:text-white uppercase tracking-tight">{{ $categories[(string)$category] ?? 'Global Heritage' }}</p>
                        </div>
                    </div>
                            ...
                            @foreach($categories as $key => $label)
                                <a href="{{ route('cultural-hub.index', ['category' => $key]) }}"
                                   class="flex items-center justify-between px-8 py-4 hover:bg-amber-500/10 transition-colors group">
                                    <div class="flex items-center space-x-4">
                                        <div class="w-2 h-2 rounded-full {{ $category === $key ? 'bg-amber-500 shadow-[0_0_12px_rgba(245,158,11,0.8)]' : 'bg-stone-200 dark:bg-stone-700' }}"></div>
                                        <span class="text-xs font-black uppercase tracking-widest {{ $category === $key ? 'text-amber-600' : 'text-stone-600 dark:text-stone-400' }}">
                                            {{ $label }}
                                        </span>
                                    </div>
                                    @if($category === $key)
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
                    @forelse($cultures as $culture)
                        <div class="bg-white dark:bg-stone-900 rounded-[2.5rem] overflow-hidden shadow-[0_15px_40px_rgba(0,0,0,0.04)] border border-stone-100 dark:border-stone-800 group flex flex-col h-full transform transition-all duration-500 hover:shadow-[0_30px_70px_rgba(0,0,0,0.1)] hover:-translate-y-1">
                            ...
                            <div class="h-48 sm:h-56 relative overflow-hidden flex-shrink-0 m-3 rounded-[2rem]">
                                @if($culture->image)
                                    <img src="{{ Storage::url($culture->image) }}" class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110">
                                @else
                                ...
                                <div class="absolute bottom-4 left-4 flex items-center bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10">
                                    ...
                                    <span class="text-[9px] font-black text-white uppercase tracking-widest">{{ $culture->submitter->name ?? 'Guardian' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="px-6 pt-2 pb-4 flex-1 flex flex-col">
                                <div class="flex items-start justify-between mb-2">
                                    ...
                                    <div class="flex items-center space-x-3 pt-1">
                                        ...
                                        @auth
                                            <button onclick="toggleCultureResonance({{ $culture->id }})" 
                                                    id="culture-vibe-btn-{{ $culture->id }}"
                                                    class="flex flex-col items-center group/vibe">
                                                <i data-lucide="zap" 
                                                   id="culture-vibe-icon-{{ $culture->id }}"
                                                   class="w-3.5 h-3.5 {{ auth()->user()->hasResonated($culture) ? 'text-orange-500 fill-orange-500' : 'text-stone-400 group-hover/vibe:text-orange-500' }} mb-0.5 transition-all"></i>
                                                <span id="culture-vibe-count-{{ $culture->id }}"
                                                      class="text-[8px] font-black {{ auth()->user()->hasResonated($culture) ? 'text-orange-600' : 'text-stone-400' }} uppercase tracking-tighter">{{ $culture->resonance_count }}</span>
                                            </button>
                                        @else
                                            <div class="flex flex-col items-center opacity-40">
                                                <i data-lucide="zap" class="w-3.5 h-3.5 text-stone-400 mb-0.5"></i>
                                                <span class="text-[8px] font-black text-stone-400 uppercase tracking-tighter">{{ $culture->resonance_count }}</span>
                                            </div>
                                        @endauth
                                        ...
                                        @auth
                                            <button onclick="toggleCultureLockin({{ $culture->id }})" 
                                                ...
                                                <i data-lucide="{{ auth()->user()->hasLockedIn($culture) ? 'check-circle' : 'lock' }}" 
                                                   id="culture-lock-icon-{{ $culture->id }}"
                                                   class="w-3.5 h-3.5 {{ auth()->user()->hasLockedIn($culture) ? 'text-amber-500' : 'text-stone-400 group-hover/lock:text-amber-500' }} mb-0.5 transition-all"></i>
                                                <span id="culture-lock-count-{{ $culture->id }}"
                                                      class="text-[8px] font-black {{ auth()->user()->hasLockedIn($culture) ? 'text-amber-600' : 'text-stone-400' }} uppercase tracking-tighter">{{ $culture->locked_in_count }}</span>
                                            </button>
                                        @else
                                            <div class="flex flex-col items-center opacity-40">
                                                <i data-lucide="lock" class="w-3.5 h-3.5 text-stone-400 mb-0.5"></i>
                                                <span class="text-[8px] font-black text-stone-400 uppercase tracking-tighter">{{ $culture->locked_in_count }}</span>
                                            </div>
                                        @endauth
                                    </div>
                                </div>
                                ...
                                <p class="text-amber-600 dark:text-amber-500 text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] mb-3">
                                    {{ $culture->category }} • {{ $culture->region }}
                                </p>
                    ...
                    @empty
                        ...
                        <div class="col-span-full text-center py-24 backdrop-blur-md bg-stone-50/50 dark:bg-stone-900/10 rounded-[4rem] border-4 border-dashed border-stone-200 dark:border-stone-800">
                            ...
                            <a href="{{ route('cultural-hub.create') }}" class="inline-flex items-center space-x-3 px-10 py-5 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-[2rem] font-black uppercase tracking-widest text-xs hover:scale-110 transition-transform shadow-2xl shadow-amber-500/40">
            ...
            <aside class="hidden lg:block lg:col-span-3 sticky top-28 space-y-10 lg:order-last">
                <div class="backdrop-blur-2xl bg-white/70 dark:bg-stone-900/70 rounded-[3rem] border border-white/40 dark:border-stone-800/50 p-10 shadow-2xl shadow-amber-500/5">
                    ...
                    <form action="{{ route('cultural-hub.index') }}" method="GET" class="mb-10 p-1.5 bg-stone-100/50 dark:bg-stone-800/50 rounded-2xl flex items-center border border-stone-200 dark:border-stone-700 focus-within:border-amber-400 focus-within:ring-8 focus-within:ring-amber-400/5 transition-all group">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="SEARCH LEGENDS..." class="bg-transparent border-none focus:ring-0 text-[10px] font-black text-stone-800 dark:text-stone-200 w-full placeholder:text-stone-400 uppercase tracking-[0.2em]">
                        <button type="submit" class="hidden"></button>
                    </form>
                    ...
                    <div class="space-y-3">
                        @foreach($categories as $key => $label)
                            <a href="{{ route('cultural-hub.index', ['category' => $key]) }}"
                                class="flex items-center justify-between group px-6 py-4 rounded-[1.5rem] transition-all duration-500 {{ $category === $key ? 'bg-stone-900 dark:bg-white text-white dark:text-stone-900 shadow-2xl scale-[1.05] z-10' : 'text-stone-500 dark:text-stone-400 hover:bg-white dark:hover:bg-stone-800 hover:shadow-xl hover:scale-[1.02]' }}">
                                <div class="flex items-center space-x-4">
                                    @php
                                        $icon = match($key) {
                                            'all' => 'globe-2',
                                    ...
                                    @endphp
                                    <i data-lucide="{{ $icon }}" class="w-5 h-5 {{ $category === $key ? 'text-amber-500' : 'text-stone-400 group-hover:text-amber-500' }} transition-colors"></i>
                                    <span class="text-[11px] font-black uppercase tracking-tight">{{ $label }}</span>
                                </div>
                ...
                <div class="bg-gradient-to-br from-amber-500 to-orange-600 rounded-[3rem] p-10 text-white shadow-2xl shadow-amber-500/30 group overflow-hidden relative">
                    ...
                    <div class="relative z-10">
                        ...
                        <a href="{{ route('cultural-hub.create') }}" class="w-full inline-flex items-center justify-center py-5 bg-white text-amber-600 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-stone-900 hover:text-white transition-all duration-300 transform group-hover:scale-105 active:scale-95">
                            ENSHRINE NOW
                        </a>
                    </div>
                </div>
