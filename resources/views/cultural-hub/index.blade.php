...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-10 sm:mb-12">
        ...
        <div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-stone-100 hover:shadow-lg transition-all duration-300 group">
            ...
            <div class="p-4 sm:p-6">
                <div class="flex items-start justify-between mb-2 sm:mb-3">
                    <div>
                        <h3 class="font-bold text-stone-800 text-base sm:text-lg mb-0.5 sm:mb-1">{{ $culture->name }}</h3>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-3 sm:space-x-4 text-[11px] sm:text-sm text-stone-500">
                        <span class="flex items-center space-x-1">
                            ...
                            <span>{{ $culture->locked_in_count }} Locked-In</span>
                        </span>
                        <span class="flex items-center space-x-1">
                            <i data-lucide="heart" class="w-3 h-3"></i>
                            <span>{{ $culture->resonance_count }} Resonance</span>
                        </span>
                    </div>
                    ...
                    <button onclick="toggleCultureLockin({{ $culture->id }})" 
                            ...
                            class="flex items-center space-x-1.5 sm:space-x-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium text-xs sm:text-sm transition-all duration-200 {{ auth()->user()->hasLockedIn($culture) ? 'bg-gradient-to-r from-amber-500 to-orange-600 text-white shadow-sm' : 'bg-stone-100 text-stone-600 hover:bg-stone-200' }}">
                        <i data-lucide="lock" class="w-3 h-3"></i>
                        <span>{{ auth()->user()->hasLockedIn($culture) ? 'Locked-In' : 'Lock-In' }}</span>
                    </button>
                    @else
                    <a href="{{ route('login') }}" class="flex items-center space-x-1.5 sm:space-x-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium text-xs sm:text-sm bg-stone-100 text-stone-600 hover:bg-stone-200 transition-all duration-200">
                        <i data-lucide="lock" class="w-3 h-3"></i>
                        <span>Lock-In</span>
                    </a>
                    @endauth
        ...
        <div class="col-span-full text-center py-8 sm:py-12">
            ...
            @auth
            <a href="{{ route('cultural-hub.create') }}" class="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-medium text-sm sm:text-base hover:from-amber-600 hover:to-orange-700 transition-all duration-200">
                Share Your Culture
            </a>
            @else
            <a href="{{ route('register') }}" class="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-medium text-sm sm:text-base hover:from-amber-600 hover:to-orange-700 transition-all duration-200">
                Join Timeline
            </a>
            @endauth
        </div>
    ...
    <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 sm:p-8 mt-4 sm:mt-0">
        <div class="text-center">
            ...
            <a href="{{ route('register') }}" class="bg-gradient-to-r from-amber-500 to-orange-600 text-white px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-medium text-sm sm:text-base hover:from-amber-600 hover:to-orange-700 transition-all duration-200 shadow-sm hover:shadow-md">
                Join Timeline
            </a>
        </div>
    </div>
